  ...
  try {
    ...
    for (const result of batchData.results) {
      if (result.success && result.data && result.data.length > 0) {
        const { predictedUnits, riskLevel } = parsePredictionData(result.data[0]);
    ...
    samplePredictions.forEach(pred => {
      console.log(`   ${pred.product?.name || pred.productId}: ${pred.currentStock} → ${pred.stockPredicted} units (${pred.riskLevel})`);
    });
    ...
    criticalProducts.forEach(pred => {
      ...
      console.log(`   ${pred.product?.name || pred.productId}: Only ${pred.currentStock} units, need ${pred.stockPredicted} (shortfall: ${shortfall})`);
    });
